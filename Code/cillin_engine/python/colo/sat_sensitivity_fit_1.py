...
def visualize(hues, thresholds, sensitivities, kde_func, gmm_func, spline_func):
    ...
    plt.tight_layout()
    plt.savefig('map1/saturation_sensitivity.png', dpi=150)
    plt.show()
...
def main():
    ...
    # 读取数据
    try:
        hues, thresholds = load_saturation_data("../../html/ColMap1.txt")
    except FileNotFoundError:
        print("错误：找不到 ColMap1.txt 文件")
        print("请确保文件格式为：")
        print("0.0:0.15")
        print("30.0:0.25")
        print("...")
    ...
    # 导出GMM参数
    export_gmm_params(gmm, "map1/sat_sensitivity_gmm.txt")
